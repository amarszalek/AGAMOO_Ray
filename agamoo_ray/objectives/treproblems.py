...
class RE21(Objective):
    ...
    def evaluate(self, x):
        ...
        if self.obj == 0:
            time.sleep(0.001*x.shape[0]/5)
            return L * ((2 * x[:,0]) + np.sqrt(2.0) * x[:,1] + np.sqrt(x[:,2]) + x[:,3])
        elif self.obj == 1:
            time.sleep(0.0012*x.shape[0]/5)
...
class RE33(Objective):
    ...
    def evaluate(self, x):
        ...
        if self.obj == 0:
            ...
            time.sleep(0.0016*x.shape[0]/5)
            # Second original objective function
            return ((9.82 * 1e6) * (x2 * x2 - x1 * x1)) / (x3 * x4 * (x2 * x2 * x2 - x1 * x1 * x1))
        elif self.obj == 2:
            time.sleep(0.0081*x.shape[0]/5)
...
class RE61(Objective):
    ...
    def evaluate(self, x):
        if self.obj == 0:
            ...
            time.sleep(0.0042*x.shape[0]/5)
            return 250 * 2289 * np.exp(-39.75*x[:, 1]+9.9*x[:, 2]+2.74)
        elif self.obj == 4:
            time.sleep(0.0044*x.shape[0]/5)
            return 25 * (1.39 /(x[:, 0]*x[:, 1]) + 4940*x[:, 2] -80)
        elif self.obj == 5:
            time.sleep(0.0480*x.shape[0]/5)
